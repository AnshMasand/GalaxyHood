...
function StockChart({ticker}) {
  ...
  useEffect(() => {
    ...
    async function fetchChartData() {
      ...
      new Date(result.label).toLocaleDateString()
      );
      const prices = data.map((result) => result.close);
      console.log(labels);
      console.log(prices);
  ...
  return (
    <div className="chart-container">
      <div className="value-summary">
        <h2>{stockName}</h2>
        <h2>${addCommas(Number(currentPrice).toFixed(2))}</h2>
        <p>
      ...
      <div className="buying-power-container">
        <h4>Buying Power</h4>
        <h3>${addCommas(Number(portfolio?.balance).toFixed(2))}</h3>
      </div>
    </div>
  );
}
